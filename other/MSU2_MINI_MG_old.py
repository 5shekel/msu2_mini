...
def show_PC_Screen():  # 显示照片
    global State_change, Screen_Error, Device_State, screenshot_test_time, screenshot_test_frame, screenshot_last_limit_time, wait_time
    if (State_change == 1):
        State_change = 0
        Screen_Error = 0
...
def UI_Page():  # 进行图像界面显示
    ...
    global machine_model, State_change
    # 创建主窗口
    root = tk.Tk()  # 实例化主窗口
...
def Get_MSN_Device():  # 尝试获取MSN设备
    global Device_State, ADC_det, ser, State_change, machine_model, My_MSN_Device, My_MSN_Data, Screen_Error, Label1, LCD_Change_now, Text1
    port_list = list(serial.tools.list_ports.comports())  # 查询所有串口
    if len(port_list) == 0:
        print('未检测到串口,请确保设备已连接到电脑')
        # Label1.config(text="设备已连接",bg="GREEN")
        time.sleep(1)
        if not MG_daemon_should_stop: Label1.config(text="设备未连接", fg="white", bg="RED")
        Device_State = 0  # 未能连接
        ...
        if (len(My_MSN_Device) >= 1 and Device_State == 0):
            ...
            if not MG_daemon_should_stop: Label1.config(text="设备已连接", fg="white", bg="green")
            LCD_Change_now = 0
            if not MG_daemon_should_stop: Text1.delete(1.0, tk.END)  # 清除文本框
...
def MSN_Device_1_State_machine():  # MSN设备1的循环状态机
    global machine_model, time_out, key_off, key_on, State_change, s1, s2, s3, color_use, Label2, photo_path1, photo_path2, write_path1, write_path2, LCD_Change_use, LCD_Change_now, write_path3, write_path4, Img_data_use
