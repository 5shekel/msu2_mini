...
def UI_Page():  # 进行图像界面显示
    ...
    global machine_model, State_change, custom_selected_names, custom_selected_displayname, full_custom_template
    # 创建主窗口
    root = tk.Tk()  # 实例化主窗口
    ...
    def show_custom():
        if hardware_monitor_manager is None:
            tk.messagebox.showerror(message='Libre Hardware Monitor 加载失败，不能自定义')
            return
        ...
        def show_error():
            tk.messagebox.showinfo(message=full_custom_error)
        ...
        def example(i):
            ...
            if i == 1:
                full_custom_template = '\n'.join([
                    ...
                    'c #ff3333',
                    'f resource/Orbitron-Bold.ttf 22',
                    'm 16 16',
                    'v 1 {:.0f}',
                    'p %',
                    'm 96 16',
...
def MSN_Device_1_State_machine():  # MSN设备1的循环状态机
    global machine_model, last_adc_check, key_eff, key_on, State_change, s1, s2, s3, color_use, Label2, photo_path1, photo_path2, write_path1, write_path2, LCD_Change_use, LCD_Change_now, write_path3, write_path4, Img_data_use
